...
def extract_number(value, decimals):
    ...
    return round(float(m.group()), decimals) if m else None

def count_gbd_gbs(value):
    if pd.isna(value):
        return 0
    return len([v for v in re.split(r"[ ,;/]+", str(value)) if "GBD" in v or "GBS" in v])

# --- Main Processing ---
if post_file is not None:
    xls_post = pd.ExcelFile(post_file)
    post_sheet = st.selectbox("Select POST sheet", xls_post.sheet_names)
    ...
    # --- Merge Project + All GRE Prod Orders + Latest Date/Time ---
    if demand_file is not None:
        ...
        if "GRE Prod Order" in demand_df.columns and "Project" in demand_df.columns:
            ...
            # Keep latest date per GRE Prod Order
            cols_to_agg = {"Project": "first"}
            if "Date / time" in demand_df.columns:
                cols_to_agg["Date / time"] = "max"

            demand_unique = demand_df.groupby("GRE Prod Order", dropna=False).agg(cols_to_agg).reset_index()
            ...
            # Map All GRE Prod Orders per Project
            project_po_mapping = (
            ...
            if "Date / time" in post_df.columns:
                post_df["Date / time"] = post_df["Date / time"].fillna("not found")

            # Reorder columns: Project, Date / time, All GRE Prod Orders
            cols = list(post_df.columns)
            if "Project" in cols:
                cols.insert(3, cols.pop(cols.index("Project")))
            if "Date / time" in cols:
                cols.insert(4, cols.pop(cols.index("Date / time")))
            if "All GRE Prod Orders (Project)" in cols:
                cols.insert(5, cols.pop(cols.index("All GRE Prod Orders (Project)")))
            post_df = post_df[cols]
    ...
    if tubs_file is not None:
        ...
        tubs_sheet = st.selectbox("Select TUBS sheet", xls_tubs.sheet_names)
    ...
    # --- Merge IT & Phy whs from BeamBalance ---
    if beam_balance_file is not None:
        xls_beam = pd.ExcelFile(beam_balance_file)
        beam_sheet = st.selectbox("Select BeamBalance sheet", xls_beam.sheet_names)
        ...
        if "Project" in post_df.columns and "Project" in beam_df.columns:
            for col in ["IT", "Phy whs"]:
                if col in beam_df.columns:
                    beam_grouped = (
                        beam_df.groupby("Project", dropna=True)[col]
                        .apply(lambda s: ",".join([str(v) for v in s if pd.notna(v)]))
                        .reset_index()
                    )
                    post_df = post_df.merge(beam_grouped, how="left", on="Project")
                    post_df[col] = post_df[col].fillna("Not Found")
    ...
    # Column indexes
    def col_idx(name): return list(post_df.columns).index(name) + 1 if name in post_df.columns else None
    ...
    datetime_idx = col_idx("Date / time")

    # Number & color formatting
    for r in range(2, ws.max_row + 1):
        if beam_idx: ws.cell(r, beam_idx).number_format = "0.00"
        if weft_idx: ws.cell(r, weft_idx).number_format = "0.00"
        if sum_idx: 
            c = ws.cell(r, sum_idx); c.number_format = "0.00"; c.font = Font(name="Aptos Narrow", size=9, color="006400")
        if waste_gre_idx:
            c = ws.cell(r, waste_gre_idx); c.number_format = "0.00"; c.font = Font(name="Aptos Narrow", size=9, color="006400")
        if action_idx:
            c = ws.cell(r, action_idx); c.number_format = "0.000"; c.font = Font(name="Aptos Narrow", size=9, color="FF0000")
        if datetime_idx:
            c = ws.cell(r, datetime_idx)
            if c.value != "not found" and pd.notna(c.value):
                c.value = pd.to_datetime(c.value)
